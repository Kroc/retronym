...
Source code is defined as Unicode UTF-8 input. This must be broken into functional units of text known as _tokens_. These are largely analogous with the words in a text-document, but there are more specific rules involved.

There are five tokenisation rules. Excepting the _punctuation rule_, all _tokens_ begin after _white-space_, defined as: ASCII space, horizontal-tab, carriage-return and/or line-feed (collectively know as 'new-line' or 'end-of-line'), and the beginning / end of input.

* **Standard Rule** :  _Tokens_ begin and end between either white-space, or other _tokens_ from the _punctuation rule_ below

* **Punctuation Rule** : The _token_ occurs between other _tokens_, with or without separating white-space

* **End-of-Line Rule** : The _token_ begins after white-space (mandatory) and continues to the end-of-line, including any white-space along the way

* **Wrapping Rule** : The _token_ begins and ends at specified markers, including any white-space along the way, but does not exceed the end of the line

* **Block Rule** : The _token_ begins and ends at specified markers and includes all white-space inbetween, even new-lines
...
Note that an empty string (`""`) is inavlid.

There are **no** string escapement sequences, for example "`\n`" used to represent a new-line in many programming languages.
...
Because strings are just bytes you can freely intermix them with numerical data, allowing you to compose non-printable character codes, such as the new-line character and to include speech-marks:
    ...
    "the ghost said ", 34, " boo!", 10

See the [section on text mapping](#text_mapping) for remapping string characters to alternative values.

### Comments ###

Comments serve as documentation for readers of your source code. They follow the _end-of-line tokenisation rule_ (note that white-space is mandatory before a comment). Implementations may discard them once tokenised.
...
Assembly language mnemomics follow the _standard tokenisation rule_. This section does not detail the allowed parameters for the numerous assembly instructions, just the keywords reserved for tokenisation.
...
|-----------|-------------------|----------------------------------------------|
| `adc`     | Z80               | Add to Accumulator with Carry
| `add`     | Z80               | Add to Accumulator
| `and`     | Z80               | Logical AND with Accumulator
| `bit`     | Z80               | Test Bit
...
| `inc`     | Z80               | Increment
| `ind`     | Z80*              | Input from Port and Decement
...
| `IX`      | Z80*              | Index-X Register
| `IY`      | Z80*              | Index-Y Register
...
The _hint_ is purely discriptive and does not affect assembly. A different _hint_ can be used for every _register_ use to document the changing role of the _register_ in your assembly code.
...
The following operators use the _punctuation tokenisation rule_ and therefore can be separated from other _tokens_ with or without whitespace.
...
A _composite identifier_ is the combination of multiple _identifiers_ separated by a period, for example: `label.subroutine`. These are used to reference symbol heirarchies.

The following regular expression describes a valid _composite indentifier_ (sans sigil):
...
Any _token_ that does not match any of the above _lexical elements_ is a critical error and should stop further processing.
